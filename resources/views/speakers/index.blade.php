...
@section('content')
    <div class="w-full px-4 py-6 mx-auto mt-16">
        ...
        @include('speakers.components.cards', [
            'totalSpeakers' => $totalSpeakers,
            'activeSpeakers' => $activeSpeakers,
            'inactiveSpeakers' => $inactiveSpeakers,
        ])
        ...
        <div class="w-full mt-6 bg-white rounded-lg shadow-lg overflow-hidden">
            <div class="p-6">
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                    ...
                    <div class="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
                        ...
                        <div class="relative flex-1 sm:flex-initial sm:w-80">
                            <div class="relative">
                                ...
                                <button type="button" id="clearSearchBtn" onclick="clearSearch()"
                                    class="hidden absolute inset-y-0 right-0 pr-3 items-center text-gray-400 hover:text-gray-600">
                        ...
                        <a href="{{ route('speakers.create') }}"
                            class="bg-primary-600 w-40 sm:w-auto hover:bg-primary-700 text-white font-bold py-3 px-3 rounded-lg shadow-lg transition duration-300 inline-flex items-center justify-center whitespace-nowrap">
                            <i class="fas fa-plus mr-2"></i> Agregar Ponente
                        </a>
                    </div>
                </div>
                ...
                <div id="searchIndicator"
                    class="hidden mb-4 p-3 bg-blue-50 border-l-4 border-blue-500 rounded items-center justify-between">
                ...
                <div id="tableContainer">
                    @include('speakers.components.table', ['speakers' => $speakers])
                </div>
            </div>
        </div>
...
@push('scripts')
    <script>
        ...
        function performSearch(searchValue, page = 1) {
            ...
            fetch(`/speakers/search?search=${encodeURIComponent(searchValue)}&page=${page}`, {
                    method: 'GET',
                    headers: {
        ...
        function toggleStatus(id) {
            fetch(`/speakers/${id}/toggle-status`, {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                        'Accept': 'application/json',
